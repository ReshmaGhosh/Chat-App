...
class MessageRVAdapter(private val messageList: ArrayList<MessageRVModel>) :
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (sender){
            "user" -> (holder as UserMessageViewHolder).userMsgTV.setText(messageList.get(position).messagr)
            "bot" -> (holder as BotMessageViewHolder).botMsgTV.setText(messageList.get(position).messagr)
        }
    }
